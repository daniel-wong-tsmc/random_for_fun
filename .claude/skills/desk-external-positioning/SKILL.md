...
| "Hard corroboration is enforced" | **Do not claim** | Charter Part 26's hard corroboration + hard secondary-confidence cap are explicitly deferred (line 1637). What is live is F63's staged 3-distinct-publisher bounded step — and F72 (open) shows that step is exploitable by cross-domain wire syndication today |
...
| Charter Part 37 line ~1637's "Not yet: hard corroboration..." list, presented as settled | **Flag, do not resolve** | It contradicts the F63 amendment ~60 lines above it (lines 1574–1586). This is a maintainer-acknowledged unresolved internal contradiction as of 2026-07-05 — an external write-up should note the contradiction exists, not silently pick a side |
...
- **Silently resolving the Part 37 line-1637 contradiction** one way or the other in an external write-up — the correct move is to name the contradiction, not adjudicate it.
...
| Charter Part 37 contradiction still present | `Select-String -Context 2,2 'Not yet \(deferred' docs/agent-swarm-charter.md` (expect line ~1637) vs `Select-String -Context 2,2 'F63 \(contract v1.3\)' docs/agent-swarm-charter.md` (expect line ~1575) |
