...
| **DOCTRINE** | `docs/agent-swarm-charter.md` amendments, binding skill prose (`run-cycle`, `gather-category`, `run-eval`), `docs/roadmap.md` constraints | Feature path (§5) + user approval; amendments must reconcile the Parts they touch | Live counter-example of failing to reconcile: charter Part 37 line ~1637 still says "Not yet: hard corroboration + a hard secondary-confidence cap" while the F63 amendment ~60 lines above it (lines ~1574–1586) shipped exactly the staged corroboration step. **Unresolved internal contradiction — flag it when citing Part 37, do not silently resolve it either way** |
...
- **Quoting charter Part 37's "Not yet" deferred list** without flagging that it contradicts the F63 amendment above it (§1, DOCTRINE row).
