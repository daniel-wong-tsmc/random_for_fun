...
# F67 Task 8: canonical inverted-pyramid order — everything above reader.APPENDIX_DIVIDER
# passes the reader contract (no raw ids/acronyms); everything below is technical detail.
SECTION_HEADERS = [
    "CATEGORY REPORT",
    "STATE OF THE MARKET",
    "WHAT MOVED",
...
def test_cli_report_exits_zero_with_all_section_headers():
    """Basic smoke test: exit 0, all canonical section headers present AND in the F67
    Task 8 inverted-pyramid order (the divider separates the reader-contract top half
    from the technical-detail appendix).
    ...
    now live only in the appendix raw-index table (this fixture has no theses
    ...
    idx = [result.stdout.index(header) for header in SECTION_HEADERS]
    assert idx == sorted(idx), "section headers are out of the pinned F67 order"
