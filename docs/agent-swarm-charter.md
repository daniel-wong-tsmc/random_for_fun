...
**Not yet (deferred, by decision):** the **staged** corroboration path (v1.3 F63 + v1.4 F72) is the
current standing rule — the remaining deferral is a **normalized/shingle near-dup** similarity pass
(v1.4 ships registry + **exact-hash** collapse; wrapper-different reprints from *unknown* syndicators
still count as distinct — contract-v1.4 §7 Q6); a **hard** secondary-confidence cap beyond the F3
badge; **unattended scheduling** (Part 28 — v1 is **manually invoked** from an open session); and a
standalone non-session web fetcher. None require a redesign — scheduling is "run this same action on a
timer." (This clause was reconciled with the F63/F72 corroboration doctrine above in the SAME change
that landed contract v1.4 — the earlier "hard corroboration deferred" wording contradicted the F63
amendment for weeks; see charter-hygiene note in `docs/migrations/2026-07-contract-v1.4.md`.)
...
| **4-4** | **Daily gather + relevance/lint engine + the brain-driven wiki ingest + the discovery engine**: recent-news + live-price daily sweep; dedup vs the store; the **richer multi-factor materiality model** (the wiki `lint`: new-thread \| thread-state-change \| contradicts-thesis[highest] \| moves-indicator∝magnitude; weighted by tier+recency; decays as a thread goes quiet); salience-decay computation; every drop **logged, never silent** (Part 29). **Includes the brain-ingest seam** (`--emit-prompt` + a wiki-ingest skill step that routes a finding to its page(s), writes the prose body, flags contradictions) — **deliberately deferred out of 4-1** (see below). **OWNS the "research topics we didn't define" requirement:** the **discovery lane** (Part 18 — the brain opens *provisional* entity/theme threads for topics not in any registry, confidence-capped + quarantined from canonical, promoted only on persist+corroborate per Part 10) and **bounded rabbit-holing** (Part 37 follow-the-trail with brakes: max rounds/docs/fan-out, on-topic filter, stop-when-dry, every cap logged), driven by an `explore` budget on the assignment. | **CORE — this effort, built after 4-3** |
| **4-5** | **Market-State brief + daily diff**: the report renders **both views** (a two-column demand/supply signal board *and* the causal driver/constraint tree naming the specific binding constraint), **leading with "what moved today"** + thread trajectories + per-signal recency; extends A's `report.py`. The committed human-output design target is [`docs/superpowers/specs/2026-06-29-human-market-brief-design-target.md`]. | **this effort — after 4-4** |
...
**Re-scoping (2026-06-29, recorded):** 4-4 and 4-5 are **back in the active sequence** (this effort), not deferred.
The trigger was an explicit user requirement: the agent must research **not only the topics we define (4-2's
registry) but also topics we *didn't* define, and rabbit-hole to the bottom of them.** That capability — the
**discovery lane** (provisional threads for undefined topics) + **bounded deep investigation** (follow-the-trail
with brakes) — is assigned to **4-4** (its substrate already exists: 4-1's provisional threads + the Part-37
gather that already trails leads). 4-2 stays scoped to the **defined** indicators (requirement 1); the discovery
seam is **not** pulled forward into 4-2. Build order is the full umbrella sequence: **4-1 (done) → 4-2 → 4-3 →
4-4 → 4-5.**
